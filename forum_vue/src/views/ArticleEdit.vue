<template>
  <div v-loading="fetching">
    <el-button text @click="$router.back()">← 返回</el-button>

    <el-card v-if="loaded" class="form-card">
      <h2 class="title">編輯文章</h2>

      <el-form ref="formRef" :model="form" :rules="rules" label-position="top" @submit.prevent="onSubmit">
        <el-form-item label="標題" prop="title">
          <el-input v-model="form.title" maxlength="255" show-word-limit />
        </el-form-item>

        <el-form-item label="內容（至少 10 字）" prop="content">
          <el-input v-model="form.content" type="textarea" :rows="12" />
        </el-form-item>

        <el-button type="primary" :loading="loading" native-type="submit">更新</el-button>
      </el-form>
    </el-card>
  </div>
</template>

<script setup>
import { ref, reactive, onMounted } from 'vue'
import { useRoute, useRouter } from 'vue-router'
import { getArticle, updateArticle } from '@/api/article'
import { useAuthStore } from '@/stores/auth'
import { ElMessage } from 'element-plus'

const route = useRoute()
const router = useRouter()
const auth = useAuthStore()

const formRef = ref()
const fetching = ref(false)
const loaded = ref(false)
const loading = ref(false)
const form = reactive({ title: '', content: '' })

const rules = {
  title: [{ required: true, message: '標題必填', trigger: 'blur' }],
  content: [
    { required: true, message: '內容必填', trigger: 'blur' },
    { min: 10, message: '至少 10 字', trigger: 'blur' }
  ]
}

onMounted(async () => {
  fetching.value = true
  try {
    const article = await getArticle(route.params.id)
    if (article.userId !== auth.currentUserId) {
      ElMessage.error('沒有權限編輯此文章')
      router.push('/')
      return
    }
    form.title = article.title
    form.content = article.content
    loaded.value = true
  } finally {
    fetching.value = false
  }
})

async function onSubmit() {
  await formRef.value.validate()
  loading.value = true
  try {
    await updateArticle(route.params.id, form)
    ElMessage.success('文章更新成功')
    router.push(`/articles/${route.params.id}`)
  } finally {
    loading.value = false
  }
}
</script>

<style scoped>
.form-card {
  margin-top: 12px;
}
.title {
  margin: 0 0 20px;
  font-size: 22px;
}
</style>
